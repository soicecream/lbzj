<template>
  <div style="margin: 0px 0px 15px 0px;font-size: 14px;position: relative">
    <el-row class="header" id="js-right-header">
      <el-col :xs="24" :sm="16" :md="16" :lg="16">
        <div class="select-row">
          <span>语言:</span>
          <span>
            <el-select :value="this.language" @change="onLangChange" class="left-adjust" size="small">
              <el-option v-for="item in languages" :key="item" :value="item">{{ item }}</el-option>
            </el-select>
          </span>
          <span>
            <el-tooltip content="重置" placement="top">
              <el-button icon="el-icon-refresh" @click="onResetClick" size="small"/>
            </el-tooltip>
          </span>
        </div>
      </el-col>
      ...
      <el-col :xs="24" :sm="8" :md="8" :lg="8">
        <div class="select-row fl-right">
          <!--          上传-->
          <span>
            <el-tooltip content="上传代码" placement="bottom">
              <el-button icon="el-icon-upload" @click="onUploadFile" size="small"/>
            </el-tooltip>
          </span>
            ...
            <el-tooltip content="代码编辑器设置" placement="top">
              <el-popover placement="bottom" width="300" trigger="click">
                ...
                <div class="setting-item">
                  <span class="setting-item-name"><i class="fa fa-tachometer"/> 主题</span>
                  <el-select :value="this.theme" @change="onThemeChange" class="setting-item-value" size="small">
                    <el-option v-for="item in themes" :key="item.label" :label="item.label"
                               :value="item.value">{{ item.label }}</el-option>
                  </el-select>
                </div>
                <div class="setting-item">
                  <span class="setting-item-name"><i class="fa fa-font"/> 字体大小</span>
                ...
                <div class="setting-item">
                  <span class="setting-item-name">
                    <svg focusable="false" viewBox="0 0 1024 1024" fill="currentColor" width="1.2em" height="1.2em"
                  ...
                  <el-select :value="tabSize" @change="onTabSizeChange" class="setting-item-value" size="small">
                    <el-option label="2个空格" :value="2">2个空格</el-option>
                    <el-option label="4个空格" :value="4">4个空格</el-option>
                    <el-option label="8个空格" :value="8">8个空格</el-option>
                  </el-select>
                </div>
              </el-popover>
            </el-tooltip>
    ...
    <el-drawer :visible.sync="openTestCaseDrawer" style="position: absolute;" :modal="false" size="40%"
      ...
      <el-tabs v-model="testJudgeActiveTab" type="border-card" style="height: 100%;">
        <el-tab-pane label="测试样例" name="input" style="margin-right: 15px;margin-top: 8px;">
          <div class="mt-10">
            <el-tag type="primary" class="tj-test-tag" size="small" v-for="(example, index) of problemTestCase"
                    :key="index" @click="addTestCaseToTestJudge(example.input, example.output, index)"
                    :effect="example.active?'dark':'plain'">
              填充样例 {{ index + 1 }}
            </el-tag>
          </div>
          <el-input type="textarea" class="mt-10" :rows="7" show-word-limit resize="none" maxlength="1000"
                    v-model="userInput"/>
        </el-tab-pane>
          ...
          <div v-loading="testJudgeLoding">
            <template v-if="testJudgeRes.status == -10">
              <div class="tj-res-tab mt-10">
                <el-alert title="没有测试判断提示" type="info" center :closable="false" show-icon/>
              </div>
            </template>
            <template v-else-if="testJudgeRes.status != -2">
              <div class="tj-res-tab">
                <el-alert class="mt-10" :type="getResultStausType(testJudgeRes.problemJudgeMode,testJudgeRes.status)"
                          :closable="false" show-icon>
                  <template slot="title">
                    <span class="status-title">
                      {{
                        getResultStatusName(testJudgeRes.problemJudgeMode, testJudgeRes.status, testJudgeRes.expectedOutput != null)
                      }}
                      <template v-if="equalsExpectedOuput != null">
                        {{ "(通过测试用例 " + equalsExpectedOuput + ")" }}
                      </template>
                    </span>
                  </template>
                  <template slot>
                    <div style="display:flex">
                      ...
                      <div>
                        <span style="vertical-align: sub;" class="color-gray mr-5">
                          <svg data-v-79a9c93e="" focusable="false" viewBox="0 0 1025 1024" fill="currentColor"
                               width="1.2em" height="1.2em" aria-hidden="true">
                            <path
                    ...
                    <div
                        v-if="testJudgeRes.problemJudgeMode == 'spj' && (testJudgeRes.status == 0 || testJudgeRes.status == -1)"
              ...
              <div class="tj-res-tab">
                ...
                <div class="tj-res-item" v-if="testJudgeRes.expectedOutput!=null">
                  <span class="name">期望输出</span>
                  <span class="value">
                    <el-input type="textarea" :readonly="true" resize="none" :autosize="{ minRows: 1, maxRows: 4}"
                              class="textarea" v-model="testJudgeRes.expectedOutput"/>
                  </span>
                </div>
                <div class="tj-res-item">
            ...
            <template v-else>
              <div class="tj-res-tab mt-10">
                <el-card>
                  <div slot="header"><span class="ce-title">编译失败</span></div>
        ...
        <el-tab-pane>
          <span slot="label">
            <el-tag type="success" class="tj-btn" @click="submitTestJudge" effect="plain">
              <i class="el-icon-video-play"> 运行样例</i>
            </el-tag>
          </span>
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  props: {
    ...
    openTestCaseDrawer: {type: Boolean, default: false,},
    pid: {type: Number,},
    type: {type: String, default: "public",},
    problemTestCase: {type: Array,},
    isAuthenticated: {type: Boolean, default: false,},
    isRemoteJudge: {type: Boolean, default: false,},
    openFocusMode: {type: Boolean, default: false,},
  },
  data() {
    return {
      options: {
